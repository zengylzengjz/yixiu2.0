...
<div class="layui-fluid">
                        ...
                        <div class="layui-card-body">
                            <div class="layui-row layui-col-space10 layui-form-item">
                                <div class="layui-col-lg12">
                                    <label class="layui-form-label">维修记录状态:</label>
                                ...
                                <div class="layui-col-lg12">
                                    <label class="layui-form-label">维修记录类型:</label>
                                    <div class="layui-input-block" v-cloak="">
                                ...
                                <div class="layui-col-lg12">
                                    <label class="layui-form-label">维修记录号:</label>
    ...
    <div class="layui-row layui-col-space15" >
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">维修记录列表</div>
                <div class="layui-card-body">
                    <div class="layui-btn-container" id="container-action">
                        <button class="layui-btn layui-btn-sm" data-type="del_order">批量删除维修记录</button>
                    </div>
                    ...
                    <script type="text/html" id="order_id">
                        <!-- lancercd -->
                        {{d.order_id}}<br/><br/>
                        <span style="color: {{d.color}};text-align:center;">
                          <!-- {{d.pink_name}} -->
                          {{#  if(d.product_type==2){ }}
                          <p><span style="color: #1abc9c;">[电脑]</span></p>
                          {{#  }else{ }}
                          {{# if(d.product_type == 1){ }}
                          <p><span style="color:#f9ca24;">[打印机]</span></p>
                          {{# }else{ }}
                          <p>
                            <span style="color: {{d.color}};">[普通记录]</span>
                          </p>
                          {{# } }}
                          {{# }; }}
                        </span><br/>
                        {{#  if(d.is_del == 1){ }}<span style="color: #ff7979;">用户已删除</span>{{# } }}
                    </script>
                    <!--用户信息-->
                    <!-- lancercd -->
                    <script type="text/html" id="userinfo">
                        {{d.real_name==null ? '暂无姓名信息':d.real_name}}/{{d.uid}}</br>{{d.user_phone==null ? '暂无电话信息':d.user_phone}}
                    </script>
                    <!--支付状态-->
                    ...
                    <script type="text/html" id="info">
                        ...
                        <div>
                            <span>
                                <img style="width: 30px;height: 30px;margin:0;" src="{{item.cart_info.productInfo.attrInfo.image}}">
                            </span>
                    ...
                    <script type="text/html" id="act">
                        ...
                        <ul class="layui-nav-child layui-anim layui-anim-upbit">
                            ...
                            <li>
                                <a href="javascript:void(0);" onclick="$eb.createModalFrame('修改维修记录','{:Url('edit')}?id={{d.id}}')">
                                    <i class="fa fa-edit"></i> 修改维修记录
                                </a>
                            </li>
                            <li>
                                <a lay-event='marke' href="javascript:void(0);" >
                                    <i class="fa fa-paste"></i>维修详情
                                </a>
                            </li>
                            <li>
                                <a href="javascript:void(0);" onclick="$eb.createModalFrame('维修记录','{:Url('order_status')}?oid={{d.id}}')">
                                    <i class="fa fa-newspaper-o"></i> 操作记录
                                </a>
                            </li>
                        </ul>
                        {{#  }else if(d._status==2){ }}
                        <!-- <button class="btn btn-primary btn-xs" type="button" onclick="$eb.createModalFrame('记录处理','{:Url('order_goods')}?id={{d.id}}',{w:400,h:250})">
                            <i class="fa fa-cart-plus"></i> 记录处理</button> -->
                            <button class="btn btn-primary btn-xs" type="button" onclick="$eb.createModalFrame('配送信息','{:Url('distribution')}?id={{d.id}}')">
                        ...
                        <ul class="layui-nav-child layui-anim layui-anim-upbit">
                            ...
                            <li>
                                <a lay-event='danger' href="javascript:void(0);">
                                    <i class="fa fa-cart-arrow-down"></i> 完成订单
                                </a>
                            </li>
                            {{# }else if(d.use_integral > 0 && d.use_integral >= d.back_integral){ }}
...
<script>
    layList.tableList('List',"{:Url('order_list',['real_name'=>$real_name])}",function (){
        return [
            {type:'checkbox'},
            {field: 'order_id', title: '维修单号', sort: true,event:'order_id',width:'15%',templet:'#order_id'},
            {field: 'nickname', title: '用户信息',templet:'#userinfo',width:'10%',align:'center'},
            {field: 'info', title: '问题描述',templet:"#info",height: 'full-20'},
            {field: 'pay_price', title: '金额',width:'8%',align:'center'},
            {field: 'paid', title: '支付状态',templet:'#paid',width:'8%',align:'center'},
            {field: 'status', title: '维修状态',templet:'#status',width:'8%',align:'center'},
        ...
        switch (event) {
            ...
            case 'marke':
                var url =layList.U({c:'order.store_order',a:'remark'}),
                    id=data.id,
                    make=data.remark;
                $eb.$alert('textarea',{title:'请输入维修详情',value:make},function (result) {
                    if(result){
                        $.ajax({
                            url:url,
            ...
            case 'danger':
                ...
                },{'title':'您确定要修改记录状态吗？','text':'修改后当前记录状态会直接变为已完成支付状态变为已支付！','confirm':'是的，我要修改'})
                break;
            case 'order_info':
    ...
    var action={
        del_order:function () {
            ...
            if(ids.length){
                ...
                },{'title':'您确定要修删除维修记录吗？','text':'删除后将无法恢复,请谨慎操作！','confirm':'是的，我要删除'})
            }else{
