...
class CombinationOrder extends AuthController
{
    ...
    public function readDelivery($id){
        ...
        $product = StoreOrderModel::get($id);
        if(!$product) return Json::fail('数据不存在!');
        Form::text('delivery_name','维修人员姓名');
        Form::text('delivery_id','维修人员电话')->number();
        return Form::builder();
    }
    ...
    public function readDistribution($id){
        ...
        if($product['delivery_type'] == 'send'){
            Form::text('delivery_name','维修人员姓名',$product->getData('delivery_name'));
            Form::text('delivery_id','维修人员电话',$product->getData('delivery_id'))->number();
