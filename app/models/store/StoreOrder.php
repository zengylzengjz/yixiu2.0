...
//        $totalPrice = bcadd($totalPrice,$storePostage,2);
    ...
    public static function getCacheOrderInfo($uid, $key)
    {
        ...
        $testMyInfo = Cache::get($cacheName);
...
//        }
    ...
    public static function getUserOrderSearchList($uid, $type, $page, $limit, $search)
    {
        ...
        foreach ($list as $k => $order) {
            ...
            $id=$order['id'];
            $pid=self::where('id',$id)->field('product_type')->find()->toArray();
            $tid=self::where('id',$id)->field('type_id')->find()->toArray();
            if ($pid['product_type']==2){
                $list[$k]['type']=UserComputer::where('id',$tid['type_id'])->field('com_type')->find();
                if($list[$k]['type'])
                  $list[$k]['type'] = $list[$k]['type']->toArray();
                else{
                  $list[$k]['type'] = null;
                }
            }
            if ($pid['product_type']==1){

                $list[$k]['type']=UserPrinter::where('id',$tid['type_id'])->field('printer_type')->find();
                if($list[$k]['type'])
                  $list[$k]['type'] = $list[$k]['type']->toArray();
                else{
                  $list[$k]['type'] = null;
            ...
            $list[$k]['cartInfo'][0]['type']=$list[$k]['type'];
        }
        return $list;
    }
    ...
    {
        //电脑订单数量
        $data['computer_count'] = self::where('is_del', 0)->where('uid', $uid)->where('status',3)->where('product_type', 2)->count();
        //打印机订单数量
        $data['printer_count'] = self::where('is_del', 0)->where('uid', $uid)->where('status',3)->where('product_type', 1)->count();
        //订单支付没有退款 数量
