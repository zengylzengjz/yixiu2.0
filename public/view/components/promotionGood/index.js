...
Component({
    ...
    methods: {
        ...
        redirectConfirm: function(e) {
            let that = this
            let id = e.currentTarget.dataset.index;
            // console.log('点击事件的索引', id)
          wx.showToast({
            icon: 'loading',
            title: 'Loading',
            mask: true,
            ...
            postCartAdd({
              ...
              if (true) {
                wx.navigateTo({
                  url: '/pages/order_confirm/index?cartId=' + res.data.cartId + '&id=' + id,
                  success: function (res) {
                    console.log('我不信1', res)
                    wx.hideToast()
                    // success
                  },
                  fail: function (res) {
                    console.log('我不信2', res)
                    // fail
                  },
                  complete: function (res) {
                    console.log('我不信3', res)
                    // complete
                  }
                });
              } else {
                ...
                wx.getStorage({
                    ...
                    //  wx.setStorageSync( 'is_default_id',  res.data.school_num );
                    //  wx.setStorageSync('college', res.data.college );
            }).catch(err => {
        ...
        goCat: function(isPay) {
            ...
            var attrValue = 1;
            // console.log(1122, this.data.productValue[this.data.attrValue])
            // var productSelect = this.data.productValue[this.data.attrValue];
            var productSelect = this.data.attrValue
            // console.log('商品属性', productSelect);
