import { orderConfirm, getCouponsOrderPrice, orderCreate, Fa_Id } from '../../api/order.js';
import { getAddressDefault, getAddressDetail, getMachineDetail, getMachineDefault, getPrinterDetail, getPrinterDefault } from '../../api/user.js';
  ...
  data: {
    printer_items: [
      ...
      // { checked: 'true' }
        ...
        machine: { machine: false }, //电脑信息组件
        printer: { printer: false }, //打印机信息组件
        addressInfo: {}, //地址信息
        // machine: { machine: false }, //机器组件
        machineInfo: {}, //机器信息
        printerInfo: {}, //打印机信息
        pinkId: 0, //拼团id
        addressId: 0, //地址id
        ...
        is_index_id: '', //默认地址id
        defaultAddress_id: '', //默认地址id
        changeAddress_id: '', //改变地址id
        defaultMachine_id: '', //默认电脑id
        changeMachine_id: '', //电脑改变id
        defaultPrinter_id: '', //打印机id
        changePrinter_id: '', //打印机改变id
        nidaye: '你大爷',
    },
    /**
     * 授权回调事件
    ...
    onLoadFun: function() {
        ...
        this.selectComponent('#machine').getMachineList();
        this.selectComponent('#printer').getPrinterList();
    ...
    onShow: function() {
        ...
        this.setData({ textMachine: true });
        this.setData({ textPrinter: true });
    ...
    OnChangeAddress: function(e) {
        ...
        // console.log('地址改变的id：', e.detail);
        this.setData({ changeAddress_id: e.detail });
        this.getaddressInfo();
     ...
     * 选择电脑后改变事件
     */
    OnChangeMachine: function(e) {
        this.setData({ textMachine: true, machineId: e.detail, 'machine.machine': false });
        // console.log('电脑改变的id：', e.detail);
        this.setData({ changeMachine_id: e.detail });
        this.getmachineInfo();
     ...
     * 选择打印机后改变的事件
     */
    OnChangePrinter: function(e) {
        this.setData({ textPrinter: true, printerId: e.detail, 'printer.printer': false });
        // console.log('打印机改变的id：', e.detail);
        this.setData({ changePrinter_id: e.detail });
        this.getprinterInfo();
        // console.log(888, this.data)
    },
    ...
    onLoad: function(options) {
        // this.data.is_index_id = options.id;
        var that = this;
        // var is_index_id = options.id;
        // console.log('nidaye:' + that.data.is_index_id);
        // console.log('你大爷', that.data.nidaye);
        this.setData({
            is_index_id: that.options.id,
            // index_id: options.id,
        });
        console.log(that.data.is_index_id, 'index_id');
        console.log(typeof that.data.is_index_id, 1);
        if (!options.cartId) return app.Tips({ title: '请选择需要的服务' }, { tab: 3, url: 1 });
        this.setData({
        ...
        wx.getStorage({
            ...
            success: function(res) {
                // success
                // console.log(55555, 'cartId', res.data)
                that.setData({ cartId_1: res.data })
                    // console.log(4444, that.data.cartId_1)
    ...
    getaddressInfo: function() {
        var that = this;
        console.log(1111, 'addressId', that.data.addressId)
        if (that.data.addressId) {
            ...
            getAddressDefault().then(res => {
                res.data.is_default = parseInt(res.data.is_default);
                that.setData({ addressInfo: res.data || {}, addressId: res.data.id || 0, 'address.addressId': res.data.id || 0 });
                this.setData({ defaultAddress_id: res.data.id });
            })
        }
    },
    /**
    ...
    getmachineInfo: function() {
        // console.log(122, 'machine', that.data.machineId)
        var that = this;
        if (that.data.machineId) {
            getMachineDetail(that.data.machineId).then(res => {
                res.data.is_default = parseInt(res.data.is_default);
                that.setData({ machineInfo: res.data || {}, machineId: res.data.id || 0, 'machine.machineId': res.data.id || 0 });
            })
        } else {
            getMachineDefault().then(res => {
                res.data.is_default = parseInt(res.data.is_default);
                that.setData({ machineInfo: res.data || {}, machineId: res.data.id || 0, 'machine.machineId': res.data.id || 0 });
                // console.log('默认电脑id：' + res.data.id);
                this.setData({ defaultMachine_id: res.data.id });
            })
        }
    ...
    getprinterInfo: function() {
        var that = this;
        if (that.data.printerId) {
            getPrinterDetail(that.data.printerId).then(res => {
                res.data.is_default = parseInt(res.data.is_default);
                that.setData({ printerInfo: res.data || {}, printerId: res.data.id || 0, 'printer.printerId': res.data.id || 0 });
            })
        } else {
            getPrinterDefault().then(res => {
                res.data.is_default = parseInt(res.data.is_default);
                that.setData({ printerInfo: res.data || {}, printerId: res.data.id || 0, 'printer.printerId': res.data.id || 0 });
                // console.log('默认打印机id：' + res.data.id);
                this.setData({ defaultPrinter_id: res.data.id });
            })
        }
    ...
    onAddress: function() {
        this.setData({
            ...
            pagesUrl: '/pages/user_address_list/index'
                // ?cartId=' + this.data.cartId + '&pinkId=' + this.data.pinkId + '&couponId=' + this.data.couponId
        });
    },
    onMachine: function() {
        this.setData({
            ...
            pagesUrl: '/pages/user_computer_list/index'
                // ?cartId=' + this.data.cartId + '&pinkId=' + this.data.pinkId + '&couponId=' + this.data.couponId
        });
    },
    onPrinter: function() {
        this.setData({
            ...
            pagesUrl: '/pages/user_printer_list/index'
                // ?cartId=' + this.data.cartId + '&pinkId=' + this.data.pinkId + '&couponId=' + this.data.couponId
        });
    },
  SubOrder: function (e) {
    ...
    // })
    var that = this;
    // console.log(that.data.InputTarea);
    var a = that.data.mark;
    console.log('suborder', a);
    if (a.length == 0) {
      console.log('快成功了')
      wx.showToast({
        title: '详情描述不能为空!',
      ...
      var a = that.data.is_index_id

      if (Number(a) == 2) { //选定电脑维修
        if (that.data.changeMachine_id != '') { //电脑id改变了
          var fa_id = that.data.changeMachine_id; //改变电脑id
          data = {
            fa_id: that.data.changeMachine_id,
          ...
          var fa_id = that.data.defaultMachine_id;
          data = {
            fa_id: that.data.defaultMachine_id,
            addressId: that.data.addressId,
            formId: formId,
            couponId: that.data.couponId,
            payType: that.data.payType,
          ...
          // Fa_Id(fa_id).then(res => {
          //     console.log('电脑id发送成功', res);
          // });
        }
      } else if (Number(a) == 1) {
        if (that.data.changePrinter_id != '') { //打印机id改变了
          var fa_id = that.data.changePrinter_id; //默认打印机id
          data = {
            fa_id: that.data.changePrinter_id,
            addressId: that.data.addressId,
            formId: formId,
            couponId: that.data.couponId,
            payType: that.data.payType,
          ...
          var fa_id = that.data.defaultPrinter_id;
          data = {
            fa_id: that.data.defaultPrinter_id,
            addressId: that.data.addressId,
            formId: formId,
            couponId: that.data.couponId,
            payType: that.data.payType,
          ...
          // Fa_Id(fa_id).then(res => {
          //     console.log('打印机id发送成功', res);
          // });
        }
      }
      // data = {
      //     fa_id: that.data.defaultAddress_id,
      //     addressId: that.data.addressId,
      //     formId: formId,
      //     couponId: that.data.couponId,
      //     payType: that.data.payType,
        ...
        wx.showModal({
          ...
          success: function (res) {
            if (res.confirm) {
              ...
              // console.log('orderkey:', this.data.orderKey);
              // console.log('data:', data);
              orderCreate(that.data.orderKey, data).then(res => {
                console.log('哼', res.data.status)
                // if (wx.showLoading) {
                var status = res.data.status,
                  orderId = res.data.result.orderId,
                  jsConfig = res.data.result.jsConfig,
                  goPages = '/pages/order_pay_status/index?order_id=' + orderId + '&msg=' + res.msg;
                console.log('哈', status)
                if (status !== 'SUCCESS') {
                  console.log('执行成功')
                  wx.showToast({
                    icon: 'loading',
                    ...
                    case 'WECHAT_PAY':
                      ...
                      wx.requestPayment({
                        ...
                        success: function (res) {
                          console.log('今天是最后一天')
                          wx.hideLoading();
                          if (that.data.BargainId || that.data.combinationId || that.data.pinkId || that.data.seckillId) return app.Tips({ title: '提交成功', icon: 'success' }, { tab: 4, url: goPages });
                          return app.Tips({ title: '提交成功', icon: 'success' }, { tab: 5, url: goPages });
                        },
                        fail: function (e) {
                          wx.hideLoading();
                          return app.Tips({ title: '取消提交' }, { tab: 5, url: goPages + '&status=2' });
                        },
                        complete: function (e) {
                          wx.hideLoading();
                          //关闭当前页面跳转至订单状态
                          if (res.errMsg == 'requestPayment:cancel') return app.Tips({ title: '取消提交' }, { tab: 5, url: goPages + '&status=2' });
                        },
                      })
                      break;
                    case 'PAY_DEFICIENCY':
