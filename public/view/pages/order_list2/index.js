...
Page({
  ...
  onLoad: function (options) {
    var that = this;
    that.getComputerOrderData1(options.id);
   ...
   * 
  */
  getComputerOrderData1: function () {
    var that = this;
    getOrderData().then(res => {
      console.log(res);
      that.setData({ ComputerOrderData: res.data });
    })
  },
  onShow: function () {
    var that = this;
    if (app.globalData.isLog) that.getComputerOrderList1(true);
  },
  onLoadFun: function () {
    this.getComputerOrderList1();
    this.getComputerOrderData1();
  },
   ...
   * 
  */
  getComputerOrderList1: function (isPage) {
    var that = this;
    if (isPage) that.setData({ loadend: false, page: 1, addressList: [] });
    that.setData({ loading: true, loadTitle: '' });
    getComputerOrderList({ page: that.data.page, limit: that.data.limit }).then(res => {
      // console.log(1);
      var list = res.data;
